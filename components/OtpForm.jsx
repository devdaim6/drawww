...
function OtpForm({ email, sendOtp }) {
  ...
  const [newPassword, setNewPassword] = useState(""); // An array to store OTP digits
  ...
  const handleVerifyOTP = async () => {
    try {
      ...
      const res = await toast.promise(
        fetch("/api/v1/user/forgot_password/verify_otp", {
          method: "POST",
          headers: {
      ...
      <section className="flex flex-col items-center justify-center min-h-screen ">
        <div className="container w-full max-w-md p-10 text-center bg-gray-300 border rounded-lg dark:border-gray-800 dark:bg-gray-700">
          ...
          <input
            type="password"
            id="password"
            className="w-full p-2 my-4 text-white bg-gray-500 border rounded-lg md:my-6 lg:my-8 dark:bg-gray-400 dark:text-black dark:placeholder:text-gray-800 dark:placeholder:opacity-60 placeholder:text-gray-300 placeholder:opacity-80"
